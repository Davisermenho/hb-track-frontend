...
import { ExerciseInput, Exercise } from '@/lib/api/exercises';
...
export function EditExerciseModal({ exerciseId, isOpen, onClose, onSuccess }: EditExerciseModalProps) {
  ...
  const initializeForm = useCallback((exerciseData: Exercise) => {
    setFormData({
  ...
  useEffect(() => {
    if (isOpen && exercise) {
      const init = () => initializeForm(exercise);
      init();
    }
  }, [isOpen, exercise, initializeForm]);

  // Update YouTube preview
  // Computar preview usando useMemo
  ...
  useEffect(() => {
    setYoutubePreview(youtubeData.preview);
    setErrors(prev => ({ ...prev, media_url: youtubeData.error }));
  }, [youtubeData]);
  ...
  const isLoading = exerciseLoading || tagsLoading;
